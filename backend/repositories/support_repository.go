...
func GetLeaderboard(limit int) ([]map[string]interface{}, error) {
	...
	rows, err := database.DB.Query(fmt.Sprintf(`
		SELECT u.id, u.email, COALESCE(NULLIF(TRIM(u.display_name), ''), '') AS display_name,
		       COALESCE(up.points, u.karma_points, 0) AS score,
		       COALESCE(up.level, 1) AS level,
		       (SELECT COUNT(*)::int FROM user_achievements ua WHERE ua.user_id = u.id) AS achievements_count,
		       COALESCE(u.avatar_url, '')
		FROM users u
		LEFT JOIN user_points up ON up.user_id = u.id
		WHERE COALESCE(up.points, u.karma_points, 0) > 0
	...
	rank := 1
	for rows.Next() {
		var id, score, level, achCount int
		var email, displayName, avatar string
		if err := rows.Scan(&id, &email, &displayName, &score, &level, &achCount, &avatar); err != nil {
			continue
		}
		lvlName := leaderboardLevelName(score)
		row := map[string]interface{}{
			"rank": rank, "user_id": id, "email": email,
			"karma_points": score, "points": score,
			"level": level, "level_name": lvlName,
...
func leaderboardLevelName(points int) string {
	switch {
	case points >= 4000:
		return "Лидер города"
	case points >= 1500:
